    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: <Widget>[
          ...
          IconButton(
            icon: const Icon(Icons.favorite), // wishlist/favourites icon
            onPressed: () {
              // Navigate to cart screen
              Navigator.push(
                context,
...
class CategoriesScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    void _addToWishlist(BuildContext context, String itemName) {
      final hashcode =context.findAncestorStateOfType<_WishlistScreenState>()?.widget.hashCode;
      final existingWishlistitems = hashcode!=null ? _WishlistScreenState._wishlistItems[hashcode]:null;// Get the current list of items in the wishlist


  // Navigate to the WishlistScreen and pass the item name
  Navigator.push(
    context,
    MaterialPageRoute(
      builder: (context) => WishlistScreen(itemToAdd: itemName ,existingWishlistitems:existingWishlistitems,),
    ),
  );
}
              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Positioned(  // Position the IconButton on top-right
        top: 8.0,
        right: 8.0,
        child: IconButton(
          icon: const Icon(Icons.favorite_border),  // Use favorite_border for wishlist
          onPressed: () {
            // Add product name to wishlist (explained later)
            _addToWishlist(context,products[index]['name']!);
          },
        ),
      ),
                  ],
...
class WishlistScreen extends StatefulWidget {
  ...
  final List<String>? existingWishlistitems; // The existing items in the wishlist



  const WishlistScreen({Key? key, this.itemToAdd ,this.existingWishlistitems}) : super(key: key);
